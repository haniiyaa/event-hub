...
interface RequestsSectionProps {
  requests: ClubJoinRequestSummary[];
  loading: boolean;
  error: string | null;
...
function RequestsSection({ requests, loading, error, onRefresh, onApprove, onReject, pendingRequestId }: RequestsSectionProps) {
  return (
    <section className="flex flex-col gap-4">
      ...
      {error ? (
        <Alert variant="error" title="Unable to load requests" description={error} />
      ) : loading ? (
                  ...
                  <tr key={request.id} className="transition hover:bg-slate-900/60">
                    ...
                    <td className="px-6 py-4 align-top">
                      <div className="flex flex-col items-end gap-2">
                        ...
                        <Button size="sm" variant="secondary" onClick={() => onReject(request.id)} disabled={isProcessing}>
...
interface ClubsSectionProps {
  ...
  error: string | null;
  filter: ClubFilter;
  onFilterChange: (next: ClubFilter) => void | Promise<void>;
  onRefresh: () => void | Promise<void>;
...
function ClubsSection({ clubs, loading, error, filter, onFilterChange, onRefresh, onUpdateStatus, pendingClubId }: ClubsSectionProps) {
  return (
    <section className="flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex flex-col gap-1">
            ...
            <span className="text-xs uppercase tracking-[0.2em] text-slate-500">
              {loading
                ? "Loading clubs…"
                : `Showing ${clubs.length} ${clubs.length === 1 ? "club" : "clubs"} (${filter.toLowerCase()} view)`}
            </span>
          </div>
          <Button variant="secondary" size="sm" onClick={onRefresh} disabled={loading}>
            Refresh
      ...
      {error ? (
        <Alert variant="error" title="Unable to load clubs" description={error} />
      ) : loading ? (
        <div className="flex min-h-[24vh] items-center justify-center">
          <Spinner />
        </div>
                  ...
                  <tr key={club.id} className="transition hover:bg-slate-900/60">
                    ...
                    <td className="px-6 py-4 align-top">
                      <div className="flex flex-col gap-1 text-slate-300">
                        ...
                        {club.admin?.username ? (
                          <span className="text-xs uppercase tracking-[0.2em] text-slate-500">@{club.admin.username}</span>
                        ) : null}
                        {club.admin?.email ? (
                      ...
                      <div className="flex flex-col items-end gap-2">
                        ...
                        {club.status === "ACTIVE" ? (
                          <Button
                            size="sm"
                            variant="secondary"
                            onClick={() => onUpdateStatus(club.id, "RETIRED")}
                        ...
                        {club.status === "RETIRED" ? (
                          <Button
                            size="sm"
                            variant="secondary"
                            onClick={() => onUpdateStatus(club.id, "PENDING")}
                            disabled={isProcessing}
                          >
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-6 py-12 sm:px-10">
      ...
      <ClubsSection
        ...
        loading={clubLoading}
        error={clubError}
        filter={clubStatusFilter}
        onFilterChange={(next) => setClubStatusFilter(next)}
